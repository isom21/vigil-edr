...
The throttle gained an optional Redis backend in PR feat/redis-ha;
these tests exercise the in-memory path (the default when
VIGIL_REDIS_URL=""). Redis-specific behaviour is in
test_redis_login_throttle.py.
"""

from __future__ import annotations


async def test_record_below_limit_does_not_block() -> None:
...
async def test_record_at_or_over_limit_blocks() -> None:
...
async def test_clear_drops_strikes_after_success() -> None:
...
async def test_different_emails_have_independent_buckets() -> None:
